...
int zlog_change_rule(const char *cname,char *level)
{
	...
	if(cname == NULL && level == NULL) return -1;

	zc_debug("------zlog_change start------");
	rc = pthread_rwlock_wrlock(&zlog_env_lock);
	if (rc) {
		zc_error("pthread_rwlock_wrlock fail, rc[%d]", rc);
		return -1;
	}
	printf("******************after changing rule******************\n");
	a_category = zc_hashtable_get(zlog_env_categories, cname);
	...
	zc_arraylist_foreach(a_category->fit_rules, i, a_rule) {
		a_rule->compare_char = compare_char;
	...
	zc_debug("------zlog_change success, total init verison[%d] ------");
	rc = pthread_rwlock_unlock(&zlog_env_lock);
	if (rc) {
		zc_error("pthread_rwlock_unlock fail, rc=[%d]", rc);
		return -1;
...
/*******************************************************************************/
/*void zlog_usr(zlog_category_t * category,const char *level,const char *format, ...)
{
	va_list args;	
